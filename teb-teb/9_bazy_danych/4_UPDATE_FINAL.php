      ...
      while ($row = $result->fetch_assoc()) {
        echo <<< USER
          <tr>
            ...
            <td><a href=./4_UPDATE_FINAL.php?updateId=$row[user_id]>█████████████</a></td>
          </tr>

USER;
      }
      echo "</table>";
//Tabela wyświetlająca KONIEC
...
//formularz dodawania KONIEC

//formularz updateowania KONIEC
if (isset($_GET['updateId'])) {
  ...
  echo <<< FORMUPDATEUSER
  <h1>Formularz edytowania</h1>
  <form method=post action=./skrypty/update.php>
  <input type=text placeholder=Imie value={$fetch['name']} name=name /><br><br>
  <input type=hidden name=user_id value={$fetch['user_id']} />
  <input type=text placeholder=Nazwisko value={$fetch['surname']} name=surname /><br>
  <br><select name=city_id>
  FORMUPDATEUSER;
  $sql = "SELECT * FROM `cities`;";
  $result = $con->query($sql);
  ...
  while ($city=$result->fetch_assoc()) {
    if ($city['city_id']==$fetch['city_id']) {
      echo "<option selected value='".$city['city_id']."'>".$city['city']."</option><br>";
    }
    else {
      echo "<option value='".$city['city_id']."'>".$city['city']."</option><br>";
    }
  ...
  <br>Data urodzenia:<input type=date value={$fetch['birthday']} name=birthday /><br><br>
  <input type=submit value=Wyślij/>
  </form>
  FORMUPDATEUSER;
}
